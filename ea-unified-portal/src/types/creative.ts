...
export interface InGameOpportunity {
  id: string;
  opportunityName: string; // e.g. "Nike x FC 27 Virtual Boot Drop & Creator Tournament"
  opportunityType: 'Brand Sponsorship / Marketing' | 'In-Game Item Sales & Bundles' | 'Player Free Rewards & Community Drops' | 'Live Event & Esports';
  tagline: string;
  targetGamerCohort: string; // e.g. "Competitive Ultimate Team & Street VOLTA Players"
  marketDemandRationale: string;
  keyDeliverables: string[]; // e.g. ["Co-branded Nike Mercurial digital kit item", "In-game stadium billboard branding", "Weekend League reward pick"]
  actionableConcept: string;
  estimatedMarketPayoff: string; // e.g. "+$3.8M In-Game Revenue & Brand Sponsorship Lift"
}
...
export interface FullAuditReport {
  ...
  inGameOpportunities?: InGameOpportunity[];
  scentOpportunities?: any[]; // Backward compatibility
  creatorSignOff?: {
