      ...
      if (rows && rows.length > 0) {
        ...
        const mappedRows = rows.map((r: any) => {
          ...
          return {
            ...
            followed_creators: r.followed_creators || telemetry?.followed_creators || [r.primary_creator_influence || "creator-smoove"],
            primary_creator_influence: r.primary_creator_influence || telemetry?.primary_creator_influence || "creator-smoove",
            favorite_club: r.favorite_club || r.p?.favorite_club || telemetry?.favorite_club || "Oklahoma City Thunder",
            favorite_player: r.favorite_player || r.p?.favorite_player || telemetry?.favorite_player || "Shai Gilgeous-Alexander",
            favorite_formation: r.favorite_formation || r.p?.favorite_formation || telemetry?.favorite_formation || "5-Out Motion Pace",
            _spanner_metadata: {
              source: "Google Cloud Spanner Enterprise (TwoKPlayerGraph)",
