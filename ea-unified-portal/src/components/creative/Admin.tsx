...
import { useAppConfig, AppConfig } from '@/context';
import { useCompanyContext } from '@/context';
import { AppMode } from '@/types';
import { AdAnalysisAdmin } from './AdAnalysisAdmin';
import { ConciergeAdmin } from './ConciergeAdmin';
...
export const Admin: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'branding' | 'navigation' | 'tables' | 'assets' | 'ad_analysis' | 'concierge' | 'gemini'>('branding');
  ...
  const createEmptyTable = () => {
    ...
    const tableName = typeof window !== 'undefined' ? window.prompt("Enter a name for the new table (without .json):", defaultTableName) : null;
    ...
    if (tables.some(t => t.id === safeName)) {
        if (typeof window !== 'undefined') window.alert("Table name already exists!");
        return;
    }
    ...
    <div className="w-full max-w-6xl mx-auto p-6 animate-fadeIn">
      ...
      <div className="flex w-full border-b border-gray-200 mb-8 overflow-x-auto">
        <button 
          ...
          className={`flex-1 justify-center py-4 text-sm font-bold flex items-center gap-2 border-b-2 transition-all ${activeTab === 'branding' ? 'border-[#349DD4] text-[#349DD4] bg-[#349DD4]/10' : 'border-transparent text-subtext hover:text-heading hover:bg-gray-50'}`}
        >
          <Palette size={18} /> Configuration
        </button>
        <button 
          onClick={() => setActiveTab('navigation')}
          className={`flex-1 justify-center py-4 text-sm font-bold flex items-center gap-2 border-b-2 transition-all ${activeTab === 'navigation' ? 'border-[#349DD4] text-[#349DD4] bg-[#349DD4]/10' : 'border-transparent text-subtext hover:text-heading hover:bg-gray-50'}`}
        >
          <Layout size={18} /> Navigation Designer
        </button>
        <button 
          onClick={() => setActiveTab('tables')}
          className={`flex-1 justify-center py-4 text-sm font-bold flex items-center gap-2 border-b-2 transition-all ${activeTab === 'tables' ? 'border-[#349DD4] text-[#349DD4] bg-[#349DD4]/10' : 'border-transparent text-subtext hover:text-heading hover:bg-gray-50'}`}
        >
          <Database size={18} /> Data Tables
        </button>
        <button 
          onClick={() => setActiveTab('assets')}
          className={`flex-1 justify-center py-4 text-sm font-bold flex items-center gap-2 border-b-2 transition-all ${activeTab === 'assets' ? 'border-[#349DD4] text-[#349DD4] bg-[#349DD4]/10' : 'border-transparent text-subtext hover:text-heading hover:bg-gray-50'}`}
        >
          <ImagePlus size={18} /> Content Assets
        </button>
        <button 
          onClick={() => setActiveTab('ad_analysis')}
          className={`flex-1 justify-center py-4 text-sm font-bold flex items-center gap-2 border-b-2 transition-all ${activeTab === 'ad_analysis' ? 'border-[#349DD4] text-[#349DD4] bg-[#349DD4]/10' : 'border-transparent text-subtext hover:text-heading hover:bg-gray-50'}`}
        >
          <Youtube size={18} /> Ad Analysis
        </button>
        <button 
          onClick={() => setActiveTab('concierge')}
          className={`flex-1 justify-center py-4 text-sm font-bold flex items-center gap-2 border-b-2 transition-all ${activeTab === 'concierge' ? 'border-[#349DD4] text-[#349DD4] bg-[#349DD4]/10' : 'border-transparent text-subtext hover:text-heading hover:bg-gray-50'}`}
        >
          <HeartHandshake size={18} /> Concierge
        </button>
      </div>
