...
export const AdAnalysisAdmin: React.FC<AdAnalysisAdminProps> = ({ editedConfig, setEditedConfig }) => {
  ...
  return (
    <div className="content-card w-full">
      ...
      {/* Real-Time Sentiment Anomaly Alert Configuration */}
      <div className="mt-10 pt-6 border-t border-gray-200">
        <h4 className="text-base font-bold text-gray-900 mb-2 flex items-center gap-2">
          ...
          Real-Time "Sentiment Anomaly Spike" Configuration
        </h4>
        <p className="text-xs text-subtext mb-4">
          Configure default statistical trigger thresholds and monitored live mechanics for the Insights Alerts engine.
        </p>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 p-5 rounded-xl border border-gray-200">
          <div>
            <label className="form-label block mb-1">Statistical Z-Score Threshold (Standard Deviations)</label>
            <input
              type="number"
              step="0.1"
              min="1.5"
              max="5.0"
              className="input-field"
              value={editedConfig.sentimentAlertThreshold || 3.0}
              onChange={e => setEditedConfig({
                ...editedConfig,
                sentimentAlertThreshold: parseFloat(e.target.value) || 3.0
              })}
            />
            <span className="text-[11px] text-gray-500 mt-1 block">Alert triggers when rolling negative sentiment crosses this standard deviation boundary.</span>
          </div>

          <div>
            <label className="form-label block mb-1">Telemetry Rage Quit / Forfeit Alert Floor (%)</label>
            <input
              type="number"
              step="1"
              min="10"
              max="100"
              className="input-field"
              value={editedConfig.forfeitAlertFloor || 25}
              onChange={e => setEditedConfig({
                ...editedConfig,
                forfeitAlertFloor: parseInt(e.target.value, 10) || 25
              })}
            />
            <span className="text-[11px] text-gray-500 mt-1 block">Correlates server log disconnects exceeding baseline in-game threshold.</span>
          </div>
        </div>
      </div>
    </div>
