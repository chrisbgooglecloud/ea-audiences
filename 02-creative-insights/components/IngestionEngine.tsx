  ...
  const initializeSimulationUsers = async () => {
    ...
    const generatedQueues = list.map((user) => {
      ...
      if (user.isAnonymous) {
        ...
        userCampaigns = categoryCampaigns.filter(c => c.type === 'Web' || c.type === 'In-App');
        if (userCampaigns.length === 0) {
          userCampaigns = activeList.filter(c => c.type === 'Web' || c.type === 'In-App');
        }
      }
