...
export const integrationAgent = {
  ...
  description: "Aggregates producer signatures and packages finalized payloads with PENDING_JUDGE_REVIEW status for live-service dispatch.",
  tools: ["signatures_key_verifier", "payload_rest_transporter"],
  dataRequired: ["key_registry", "api_endpoints"],

  async run(approvedDataJson, ai, companyName = "EA Games FC") {
    const approvedData = safeParseJson(approvedDataJson, { name: "Active Campaign", status: "Approved" });

    try {
      const response = await ai.models.generateContent({
        model: "gemini-3.5-flash-lite",
        contents: `You are the ${companyName} Integration Agent. Finalize campaign payload formatting, check signatures, and structure the outbound transit package for EA SPORTS FC 27.
        ...
        3. Formulate a dispatch verification log: "電子署名 VERIFIED. Outbound Transit Package constructed. Status: PENDING_JUDGE_REVIEW (Awaiting final critic review & franchise coordinator signoff)."
