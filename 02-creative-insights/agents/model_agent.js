...
export const modelAgent = {
  ...
  description: "Triggers Auto-ML recommend blueprints when gamer engagement or conversion deltas drop below threshold averages.",
  tools: ["bigquery_ml_builder_specs"],
  dataRequired: ["m360_historical.json"],

  async run(performanceDeltas, ai, companyName = "EA Games FC") {
    let mockM360Data = [];
    try {
    ...
    try {
      const response = await ai.models.generateContent({
        ...
        contents: `You are the ${companyName} Model Agent. Review campaign performance data and determine if custom machine learning models are recommended for training in BigQuery on gamer telemetry.
        ...
        - Recommended ML model type (e.g. XGBoost Classification for in-game churn, BigQuery Matrix Factorization for player pack recommendations, or AutoML).
